...
for exp_num in [2,3,4,5,6,7,8,9,10]:
    ...
    for iter_num in range(0,14):
        ...
        try:
            ...
            random_samples = unlabeled_df.sample(instance_num, random_state=SEED)
            random_samples.to_csv(f"{exp_folder}data/iter{iter_num+1}/{measure}.csv",index=False)
        except:
            print("failed...")
