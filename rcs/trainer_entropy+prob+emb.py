...
for exp_num in [1,2,3,4,5,6]:
    ...
    exp_folder = f"n{measure}/exp{exp_num}/"
    os.makedirs(f"n{measure}/", exist_ok=True)
    ...
    for iter_num in range(0,14):
        ...
        try:
            ...
            with torch.no_grad():
                for idx, text_case in enumerate(text):
                    ...
                    for task_index in range(len(logits_list)):
                        ...
                        task_probabilities.append(np.sqrt(768) * probabilities)  # Weighted task probabilities

                    entropy_list.append(entropy_case)
                    task_probabilities = np.concatenate(task_probabilities, axis=0)  # Concatenate task probabilities along the columns axis
                    feature_emb.append(np.hstack((task_probabilities, unlabeled_emb_np[idx])))  # Horizontally stack task probabilities and unlabeled_emb
            ...
            kmeans = KMeans(n_clusters=n_clusters, random_state=SEED).fit(feature_emb)
            unlabeled_df['cluster'] = kmeans.labels_
            print(f"Size of clusters:\n{unlabeled_df['cluster'].value_counts()}")
            log_message(f"Size of clusters: {unlabeled_df['cluster'].value_counts()}")
